...
begin
	...
	/* RELATIONS */
      ...
      UPDATE views v JOIN Staging s ON v.id = s.object_id and v.valid_to is null and s.target_table = 'views' SET v.valid_to = s.date where v.md5 != s.md5;
       ...
       INSERT INTO properties (model_id, object_id, name, documentation, valid_from,md5)
		SELECT s.model_id, s.ref_id, s.name, s.documentation, s.date,s.md5 FROM staging s
       ...
       INSERT INTO properties (model_id, object_id,name,documentation,valid_from,md5) SELECT s.model_id, s.ref_id, s.name, s.documentation, s.date,s.md5
    ...
    /* OBJECTS_IN_VIEW */
		/* new objects in view*/	
       INSERT INTO objects_in_view (model_id, object_id, view_id, valid_from)
        SELECT s.model_id, s.ref_id, s.object_id, s.date FROM staging s
        WHERE s.ref_id !='' and s.target_table='objects_in_view'
        and NOT EXISTS (SELECT 1 FROM objects_in_view oiv
        WHERE oiv.object_id = s.ref_id AND oiv.view_id = s.object_id and oiv.valid_to is null);
